...
또 다른 람다 동기식 호출인 Lambda@Edge에 대해  
다뤄보도록 하겠습니다  
여러분이 `CDN을 통해 CloudFront를 배포`했다고 가정하죠  
`각 엣지 로케이션과 함께 글로벌 람다 함수를 사용하고 싶은 경우`에는 어떡하죠?  
혹은 요청이 `애플리케이션에 도착하기 전에 필터링`하고 싶다면 어떻게 할까요?  
그럴 경우 Lambda@Edge를 사용하시면 됩니다  

Lambda@Edge는 `CloudFront CDA`를 통해  
특정 지역이 아니라 세계 각지에 람다 함수를 배포하기 위한 겁니다  
그럼 왜 쓰는 걸까요? 즉각 반응하는 애플리케이션을 위해서죠  
`여러분이 서버를 관리하지 않으므로 Lambda는 전역으로 배포`됩니다    
여러분은 CDN에 있는 모든 것을 수정하실 수 있으며  
여러분이 사용하시는 것에 대해 값을 지불하게 됩니다  
이것이 Lambda@Edge의 전제입니다  
그럼 뭘 위해 람다를 쓸 수 있을까요?  
CloudFront 요청과 응답을 바꾸기 위해서죠  
엣지에는 4가지의 람다 함수가 있습니다  
통합을 확인해 보시면 우리의 사용자는  
CloudFront 엣지 로케이션에 말을 걸고  
여기서는 오리진에 말을 걸게 됩니다 
우선 Lambda@Edge에서 수정할 수 있는 것 중 첫 번째는 바로 뷰어 요청입니다  
뷰어 요청은 CloudFront가  
뷰어, 사용자로부터 요청을 받았을 경우인데  
이 요청을 수정할 수 있는 겁니다  
수정할 수 있는 두 번째 응답 유형은 오리진 요청입니다  
이 요청은 CloudFront가 오리진으로 전달하기 전인 요청 입니다  
또 수정 가능한 세 번째 요청은 오리진 응답입니다  
CloudFront가 오리진으로부터  
응답을 받았을 경우 이를 수정할 수 있습니다  
마지막으로 뷰어 응답을 수정할 수 있습니다  
CloudFront가 뷰어에게 응답하기 이전을 말하죠  
이 말은 뷰어 요청과 뷰어 응답이라는 람다 함수를 사용할 경우  
오리진에 요청을 보내지 않고도  
뷰어에 대한 응답을  
생성할 수 있다는 의미입니다  

Lambda@Edge는 이 4가지요청을 바꿀 수 있게 해줍니다  
왜 이런 일을 할까요? 글로벌 앱을 만들었다고 가정합시다  
우리 웹사이트는 HTML 형식으로 Amazon S3에 정적 호스팅 될 겁니다  
그럼 방문한 사용자에게 클라이언트 측 JavaScript가  
CloudFront로 API 요청을 보내라고 요청하고  
CloudFront는 Lambda@Edge 함수를 가동하는데  
이는 여러분의 엣지 로케이션과 함께 글로벌하게 실행되겠죠  
그럼 람다 함수가 Amazon DynamoDB에 데이터를 쿼리합니다  
DynamoDB가 뭔지는 곧 배우게 될 겁니다  
간단히 설명하자면 서버리스 애플리케이션을 위한 글로벌 데이터베이스입니다  
그럼 왜 Lambda@Edge를 쓸까요?  
사용 사례는 어떨까요? 바로 웹사이트 보안 및 개인 정보 보호와  
엣지에 대한 다양한 웹 애플리케이션  
`검색 엔진 최적화인 SEO 때문에 사용`하는 거죠  
여러분은 `오리진과 데이터 센터 사이를 지능적으로 라우팅`할 수 있습니다  
엣지의 `Bot을 완화할 수 있고 실시간으로 이미지를 변형`할 수 있습니다    
A/B 테스팅은 물론 오리진 도달 전에 사용자를 인증하고 권한을 부여할 수도 있죠  
`사용자 우선 순위, 사용자 추적과 분석도 가능`하고요  
Lambda@Edge는 여러분이 시험 전에 심화 수준까지 아셔야 합니다  
앞서 살펴본 4가지의 요청을 수정하기 위해  
람다 함수와 CloudFront의 분배에서  
통합해야 하는 것들을 알아봤습니다  

## Lambda 비동기식 호출 및 DLQ

이제 동기식 호출을 살펴봤으니  
비동기식 호출도 살펴봅시다  
람다 함수를 뒤에서 불러오는 서비스를 위한 것으로  
`Amazon S3, SNS 주제들 CloudWatch 이벤트` 등이 있습니다  
구체적인 예시를 들어 보겠습니다

S3 버킷이 있고 새 파일에 대한 S3 이벤트 알림이 있다고 합시다  
이게 람다 서비스 안으로 이동하고  
비동기식이기 때문에 어떤 일이 일어날 건데요  
이벤트는 내부의 이벤트 대기열에 위치하게 될 겁니다  
이벤트 대기열이 여기 있고요 람다 함수는  
이 이벤트 대기열을 읽게 됩니다  
그럼 람다 함수는 이 이벤트들을 처리하려고 시도할 텐데요  
`만약 뭔가 문제가 생길 경우 람다 함수는 자동으로 재시도`를 할 겁니다  
총 세 번의 시도가 발생하는데요  
첫 번째는 바로 시도되고  
두 번째는 1분 후에  
세 번째는 두 번째 시도의 발생 후 2분 뒤에 발생됩니다  
`핵심은 람다 함수가 총 세 번의 시도를 한다는 것`입니다  
만약 이렇게 재시도가 발생하게 되면  
해당 람다 함수가 같은 이벤트의 실행을  
여러 번 시도하는 것이기 때문에  
문제가 될 수 있습니다 즉 람다 함수에 멱등성이 없다면  
큰 문제로 이어질 수 있으며 람다 함수는 멱등적이어야 합니다  
`여기서 멱등성이란, 여러 번의 시도로 나온 결과가 같음을 의미`하죠  
만약 재시도가 발생한다면  
CloudWatch 로그에서 로그 엔트리가 복제되는데  
람다 함수가 계속 재시도를 하기 때문에 그렇습니다  
`dead-letter queue, 줄여서 DLQ`를  
재시도가 끝난 뒤에 정의할 수 있는데요  
만약 처리가 실패했고  
재시도로 인해서 성공이 절대 불가능한 경우에  
람다 함수가 이벤트를  
`SQS나 SNS로 보내서 나중에 처리가 되도록 하는 거죠`  
비동기식 호출의 목적과 역할은 이렇습니다  
'왜 동기식이 아닌 비동기식을 쓰는가?'가 궁금하실 텐데  
첫 번째로 `일부 서비스에는 비동기식 호출만 사용이 가능해서 선택권이 없고요` 
두 번째 이유로는 `과정의 속도를 높여야 하는 상황에서 결과를 기다릴 필요가 없다면
1,000개의 파일이 동시에 처리되도록 해서 모든 파일이 병렬로 처리될 때까지  기다리기만 하면 됩니다`
하지만 각 결과는 기다리지 않아도 되니  
처리 시간을 줄여 주게 되는 거죠  
비동기식 서비스로는 어떤 것들이 있을까요?  
먼저, Amazon S3로 람다 함수를 호출하는 `S3 이벤트 알림`이 있죠  
`SNS의 경우에는 알림`을 받고  
`람다 함수를 트리거링`합니다  
CloudWatch 이벤트나 CloudWatch EventBridge는  
AWS 인프라에서 일어나는 일에  
람다 함수가 반응하도록 합니다  
실습에서 살펴보지 않을 서비스 중에서는  
CodeCommit이 있는데 새 브랜치, 태그, 푸시가 있을 때  
람다 함수를 트리거링합니다  
CodePipeline은 파이프라인 중에 다른 람다 함수를 호출하고  
람다 함수는 CodePipeline을 콜백해야 합니다  
또 다른 서비스로는 로그 처리용 CloudWatch 로그  
SES, 즉 이메일 전송용의 간단한 이메일 서비스  
CloudFormation, Config IoT와 IoT 이벤트가 있습니다  
우리의 관점에서 이런 인증들을 다룰 때는  
람다 함수가 Amazon S3, SNS CloudWatch 이벤트, 혹은  
EventBridge와 어떤 식으로 작동하는지를 알고 있어야 하죠  

## Lambda 및 S3 이벤트 알림

이번에는 람다를 통해서 S3 이벤트 알림을  
통합하는 방법에 대해 알아보겠습니다  
S3 이벤트 알림을 짧게 복기하자면 객체가 생성 또는 제거  
복구이나 복제가 일어날 때 알림을 해주는 기능으로  
접두어와 접미어로 필터링이 가능하고요  
Amazon S3에 업로드된 모든 이미지 파일의 썸네일을  
생성하는 등이 전형적인 사용 사례가 되겠죠  
Amazon S3에 이벤트가 있고  
S3는 이를 세 가지로 보낼 수 있는데요  
첫 번째는 SNS로 SNS 주제에서 이를  
다수의 SQS 대기열로 보내기 위해 팬아웃 패턴을 활용할 수 있죠  
SQS 대기열로 바로 보내서  
람다 함수가 직접 SQS 대기열을 읽게 할 수도 있습니다  
혹은 Amazon S3 이벤트 알림이 바로 람다 함수를  
호출하도록 할 수도 있는데  
이것은 비동기식 호출에 해당합니다  
이 람다 함수는 데이터를 이용해 어떤 작업이든 가능한데요  
그러다 문제가 생기면 Dead-letter 대기열인 SQS을 설정할 수 있겠죠  
앞서 배웠던 내용입니다  
S3 이벤트 알림은 보통 이벤트를 몇 초 만에  
전달하지만 종종 1분 이상이 걸릴 수 있습니다  
그리고 이벤트 알림을 하나도 놓치고 싶지 않다면  
버킷에 버저닝을 반드시 활성화하시기 바랍니다  
그렇지 않으면 하나의 오브젝트에 동시에 두 개의  
작성이 일어나는 경우, 하나의 알림밖에 받지 못 할 수도 있거든요  
문서에서 발췌한 부분이지만 알아 두면 좋을 내용입니다  
그럼 예시로 간단한 패턴을 살펴봅시다  
S3 버킷이 새 파일 이벤트를 람다에게 보내면  
람다 함수가 그 파일을 처리해서  
DynamoDB 테이블이나 RDS 데이터베이스의  
테이블로 보내게 되는 겁니다  

## Lambda 이벤트 소스 매핑

이번에는 이벤트 소스 매핑을 알아보겠습니다
AWS에서 람다가 이벤트를 처리하는 방법 중 마지막 유형이죠
이는 `Kinesis 데이터 스트림 SQS 및 SQS FIFO 대기열과 DynamoDB 스트림에 사용`됩니다
이들의 공통 분모를 들자면 `레코드가 소스에서 폴링`되어야 한다는 겁니다
즉 `람다가 서비스에 레코드를 요청해야 그 레코드가 반환`되는 거죠
람다가 이 `서비스들로부터 폴링을 해야 한다는 의미`입니다
이런 경우, `람다 함수는 동기적으로 호출`됩니다

예시를 한 번 살펴보죠 Kinesis와 람다 서비스입니다
`람다가 Kinesis에서 읽기를 하도록 구성`을 하면
이벤트 소스 매핑이 내부적으로 생성됩니다
그럼 이 매핑이 Kinesis를 폴링하게 되고
Kinesis에서 반환된 결과를 받아 옵니다
Kinesis가 배치를 반환하는 거죠
그리고 `이벤트 소스 매핑에 람다가 처리할 데이터`가 생기면
`이벤트 배치를 통해 람다 함수를 동기적으로 호출`할 겁니다
이게 작동 방식의 핵심이죠

이벤트 소스 매퍼에는 두 가지 범주가 있습니다
첫 번째는 `스트림` 두 번째는 `대기열`입니다

먼저 스트림은 `Kinesis 데이터 스트림`과 `DynamoDB 스트림`에 적용되는데요 이에 대해서는 곧 배우겠습니다
스트림은 `이벤트 소스 매핑이 일어나 각 샤드에 대한 반복자를 생성`합니다
Kinesis 혹은 DynamoDB 스트림 샤드에 대해서요
그리고 `샤드 레벨에서 아이템을 순차적으로 처리`합니다
따라서 `읽기 시작 위치를 구성`할 수 있게 됩니다
`새로운 아이템만 읽기`를 하거나
샤드 시작 위치 혹은 `특정한 타임스탬프부터 읽도록 구성`이 가능하죠
Kinesis에서든 DynamoDB에서든
`샤드에서 아이템이 처리될 때 아이템은 스트림으로부터 제거되지 않으며`
Kinesis와 DynamoDB의 다른 소비자들이 데이터를 읽을 수 있습니다
애초에 이런 식으로 작동하는 시스템이죠
이 부분을 다시 강조하고 싶었습니다

사용 예시로는 낮은 트래픽 또는 높은 트래픽이 있습니다
만약 트래픽 스트림이 낮다면
배치 윈도우를 사용해서 `처리 전에 레코드를 축적`합니다
이렇게 다른 람다 함수를 효율적으로 호출할 수 있죠

그리고 만약 스트림의 처리량이 매우 많고
처리 속도를 높이고 싶다면
람다가 `샤드 레벨에서 동시에 여러 배치를 처리`하도록
설정할 수 있습니다
AWS 블로그에서 가져온 도면을 함께 보시죠
샤드와 레코드 프로세서가 있고
동일한 샤드에서 다수의 람다 함수가 배치를
처리할 수 있도록 하는 `병렬적 구성이 가능`합니다
샤드당 최대 10개의 배치 프로세서를 가질 수 있으며
`파티션 키 레벨에서 각 배치가 순차적으로 처리`되게 되죠
그래서 파티션 키를 지정하면
샤드 전체를 순서대로 읽는 건 아니지만
`샤드 내의 각 키는 순차적으로 읽기`가 되죠
이런 방식으로 람다 함수와 스트림에서
병렬 처리를 할 수 있습니다

오류는 어떨까요, 기본적으로 함수가 오류를 반환하면
`함수가 성공할 때까지, 혹은 배치 내의 아이템이 만료될 때까지 모든 배치가 다시 처리`됩니다
아주 중요한 내용입니다, `배치에 오류가 생기면 처리가 중단`될 수 있어요
그래서 순차적인 처리를 위해
`오류가 해결될 때까지는 오류의 영향을 받는 배치의 처리가 중단`됩니다
이를 관리하는 방법으로는 여러 가지가 있는데요
`이벤트 소스 매핑이 오래된 이벤트를 폐기하도록 구성`할 수도 있고
또는 `재시도 횟수 제한이나 오류 시 배치 분할`을 하도록 할 수도 있습니다
이는 `람다 시간초과 문제`가 생기는 경우에 사용되죠
람다 함수가 배치 전체를 처리할 시간이 없다고 해도
배치의 절반을 처리하기에는 충분할 수도 있으니까요
그리고 오래된 이벤트를 폐기하는 경우에도
그 이벤트 전체가 목적지로 전달되는데요
목적지는 다음 강의에서 다룰 겁니다
이렇게 스트림을 살펴보았고 다음으로는 대기열을 알아봅시다

대기열은 SQS 및 SQS FIFO에 사용됩니다
여기에서도 개념은 동일한데 SQS 대기열은 람다의
이벤트 소스 매핑으로 폴링이 됩니다
그리고 배치가 반환될 때마다
`이벤트 배치를 통해 람다 함수가 동기적으로 호출`될 겁니다
`SQS의 경우, 이벤트 소스 매핑이 긴 폴링을 이용해 SQS를 폴링`합니다
효율적인 방법이죠 그리고 `배치 크기는 1에서 10메시지까지 지정이 가능`합니다
구성 화면에서 보시다시피 배치 크기와 SQS 대기열뿐이죠
AWS 웹사이트의 권장 사항에 따르면
`대기열 표시 시간초과를 람다 함수 시간초과의 여섯 배로 설정`하는 게 좋다고 하네요, 구성 가능한 내용이죠

DLQ를 사용하는 경우 즉, SQS의
`읽기와 처리에 문제가 생겨서 Dead-letter 대기열로 보내는 경우`
이때는 DLQ를 `람다가 아닌 SQS 대기열에 설정`합니다
왜 DLQ를 람다가 아닌 SQS로 설정해야 하는 걸까요?
`람다용 DLQ는 비동기식 호출에만 작동하기 때문`입니다
그런데 `지금은 동기식 호출`이죠
아니면 실패하는 경우 람다 목적지 기능을 쓸 수도 있지만
이 내용은 다음 강의에서 다루겠습니다

FIFO 대기열, 즉 선입선출을 사용하는 경우에는
`람다가 순차적 처리를 지원`하는데요
그리고 대기열을 처리하기 위해서
`스케일링 될 람다 함수의 수는 활성 메시지 그룹의 숫자와 동일`합니다
그룹 ID 설정이죠
`표준 대기열을 사용하면 아이템이 순서대로 처리되지 않을 겁니다`
표준 대기열에서 람다는 대기열의 모든 메시지를 읽을 수 있도록
최대한 빠르게 스케일링됩니다
`대기열에 오류가 발생하면 배치는 개별 아이템으로서 대기열로 반환`될 것이고
원래 배치와 다른 그룹에서 처리될 수 있습니다
가끔 이벤트 소스 매핑이
함수 오류가 일어나지 않았음에도 불구하고
대기열로부터 같은 아이템을 두 번 수신할 수도 있는데
그런 경우에 람다 함수에서
아이템이 처리되도록 해야 합니다
마지막으로 람다에서 아이템이 처리되면
람다는 대기열에서 아이템을 삭제할 것이고
이 아이템은 다시는 나타나지 않게 되죠
소스 대기열을 구성하여, 처리되지 않은 아이템들을
Dead-letter 대기열로 보낼 수 있습니다
이 내용들이 이해가 되셨길 바랍니다
설정하는 방법은 실습에서 보여드릴게요
그럼 스케일링은 어떨까요?
이벤트 매퍼의 스케일링을 다시 요약해 드리겠습니다
Kinesis 데이터 스트림과 DynamoDB 데이터 스트림에서
각 스트림 샤드당 람다 호출은 한 번입니다
병렬 처리를 사용한다면 샤드당 동시에 10개 배치까지
처리할 수 있습니다
SQS 표준에서는 람다가 매우 빠르게 스케일링된다고 했었죠
스케일 업을 위해 추가하는 인스턴스가 분당 16개니까, 제법 빠른 거죠
또한 초당 동시에 처리되는 배치량은
최대 1,000개입니다
SQS FIFO는 조금 다릅니다
그룹 ID가 같은 메시지는 무조건 순서대로 처리되고
람다 함수는 활성 메시지 그룹의 수만큼 스케일 업합니다
이들을 정의하는 것이 바로 그룹 ID입니다
여기까지 람다 이벤트 소스 매핑을 살펴봤습니다

## Lambda 목적지

`이벤트 매퍼를 실행할 때 성공인지 실패인지를 쉽게 알 수가 없다는 문제점`이 있었습니다
만약 그랬다면 데이터를 회수하게 되죠
여기서 목적지라는 개념은 `비동기화 호출 결과 또는 이벤트 매퍼의 실패 결과를 다른 어딘가로 전송`하는 것입니다
그게 어디일까요? 곧 알게 되실 겁니다
비동기식 호출에서는 `성공 및 실패한 이벤트 모두에 목적지를 정의`할 수 있는데요
여기서 성공 및 실패는 이벤트 처리 여부입니다
비동기식 호출의 `목적지로는 SQS, SNS, 람다, 그리고 Amazon EventBridge 버스인 CloudWatch 이벤트`가 있습니다
람다 함수가 비동기적으로 호출되어
예를 들어 S3 이벤트를 통해 호출되고, 그것이 성공한 경우
성공한 이벤트의 목적지로
전송할 수 있게 되고
만약 실패하면, 실패한 이벤트 목적지로 전송하는 식이죠
눈치를 채셨을 수도 있지만 `비동기식 호출의 DLQ 설정과 굉장히 유사한 형태`입니다
그래서 이제는 `DLQ 대신 목적지를 사용하는 게 권장`됩니다
동시에 둘 다 가능한 경우라도요
목적지 기능이 더욱 최신이고
더 많은 대상을 가질 수 있기 때문이죠
DLQ는 SQS와 SNS로 실패만 전송하도록 하지만
목적지 기능은 성공과 실패 결과 모두가
SQS, SNS, 람다 및 EventBridge로 전송되게끔 허용이 되니까요
여기까지 비동기식 호출의 경우였고
이벤트 소스 매핑은 어떨까요?
이 경우는 처리할 수 없는 이벤트 배치를
폐기하는 경우에 사용이 됩니다
따라서 해당 이벤트를 Amazon SQS나 Amazon SNS
전송할 수 있는 겁니다 도면에 나와 있는 내용이죠
Kinesis로부터 읽기를 해 이벤트 소스 매핑이 있을 것이고
데이터가 계속 처리되도록 시도를 하겠지만
성공하지 못 한 경우에는
Kinesis 데이터 스트림 전체를 차단하는 대신
폐기된 이벤트 배치를 실패한 이벤트 목적지로 전송하기만 하면 됩니다
그러니 이벤트 소스 매핑이 SQS에서 읽기를 하고 있다면
실패한 목적지를 설정하거나, 아니면 DLQ를 SQS에 직접
설정하시는 것 잊지 마세요
선택은 여러분의 몫입니다

## Lambda 권한 - IAM 역할 및 리소스 정책

이번에는 람다 실행 역할과 권한에 대해 알아봅시다
그동안 실습을 통해 많이 다루었기 때문에
여러분 모두 원리를 알고 계시겠지만
이론 강의로 돌아가 한 번 더 확인해 보면 좋겠죠
`IAM 역할은 람다 함수에 연결`되어 있어야 하며
이를 통해 `람다 함수는 AWS 서비스와 리소스에 액세스할 수 있는 권한`을 갖습니다
그리고 재사용이 가능한
`람다 샘플 관리 정책`도 있는데요 예를 들어 AWSLambdaBasicExecutionRole 로
CloudWatch에 로그를 업로드할 수 있습니다
또한 `Kinesis 실행 역할로 Kinesis에서의 읽기도 가능`하며
`DynamoDB 실행 역할로 DynamoDB 스트림에서의 읽기가 가능`하고
`SQS 대기열 실행 역할로 SQS에서의 읽기도 가능`하며
람다 VPC 액세스 실행 역할은
VPC 내부에 람다 함수를 배포할 수 있도록 해주죠
이번 섹션에서 살펴볼 내용입니다
또 XrayDaemon 쓰기 액세스로 Xray에 추적 데이터를 업로드할 수도 있는데
이것도 이번 섹션에서 다룰 겁니다
이들은 관리 정책으로, `자신만의 람다 정책을 만들 수도 있습니다`
`이벤트 소스 매핑으로 함수를 호출할 때마다 데이터를 읽는 것이 바로 람다`입니다
따라서 이벤트 `데이터를 읽으려면 실행 역할이 필요`한 겁니다

반면 `람다 함수가 다른 서비스를 통해 호출된 경우라면 특정한 권한을 가진 특정 IAM 역할이 필요 없습니다`
하지만 가장 좋은 방법은 지금까지 실습에서처럼
함수마다 하나의 람다 실행 역할을 생성하는 거죠
이런 정책들은 이벤트 소스 매핑을 하거나
람다 함수가 다른 서비스를 호출해야 하는 경우에 사용되는데
`람다 함수가 다른 서비스를 통해 호출되는 경우에는 리소스 기반 정책이 사용`됩니다
다른 계정 및 AWS 서비스에게 람다 리소스를 사용할 수 있는
권한을 제공하여 함수에 호출할 수 있도록 하는 거죠
Amazon S3 버킷의 S3 버킷 정책과 아주 비슷합니다
그리고 이때의 규칙은 IAM 주체가 람다 함수에
액세스할 수 있다는 건데요 이 두 가지 경우일 때 해당합니다
첫 번째로는 주체와 연결된 IAM 정책이 인증을 하는 경우로
예를 들어 IAM 사용자가 모든 권한을 가지고 있다면
관리자 액세스 정책 덕분에 이때까지 해왔던 것처럼
람다 함수에 액세스를 할 수 있죠
혹은 리소스 기반 정책을 가지고
람다 함수로 액세스 권한을 부여하는 경우라면
서비스 간 액세스일 때 더 적합할 겁니다
앞서 이미 봤었던 내용이지만
Amazon S3 같은 다른 AWS 서비스가
람다 함수를 호출하려고 한다면
리소스 기반 정책이 액세스 권한을 제공하는지 확인해야 합니다
보통은 콘솔이 뒤에서 해주는 작업이지만
직접 통합 설정을 하시려는 경우에는
직접 하셔야 하는 부분이죠

# 퀴즈 풀이

- Amazon RDS 인스턴스를 Elastic Beanstalk 환경에서 분리하려면 어떻게 해야 합니까?
  - https://aws.amazon.com/ko/premiumsupport/knowledge-center/decouple-rds-from-beanstalk/
